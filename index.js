    ...
    db.serialize(() => {
          ...
          bcrypt.hash("password123", 10, (err, hashedPassword) => {
            if (err) {
              ...
              db.run(
                ...
                ["dummy_parent", hashedPassword, "parent"],
                function (err) {
                  if (err) {
      ...
      db.get("SELECT COUNT(*) AS count FROM videos", (err, row) => {
        if (err) {
          ...
          const dummyVideos = [
            {
              title: "Match Highlights - Game 1",
              thumbnail: "https://placehold.co/300x200?text=Video+1",
              price: 5.0,
              video_url: "https://www.w3schools.com/html/mov_bbb.mp4",
            },
            {
              title: "Full Match - Game 1",
              thumbnail: "https://placehold.co/300x200?text=Video+2",
              price: 10.0,
              video_url: "https://www.w3schools.com/html/mov_bbb.mp4",
            },
            {
              title: "Training Session - Week 3",
              thumbnail: "https://placehold.co/300x200?text=Video+3",
              price: 7.5,
              video_url: "https://www.w3schools.com/html/mov_bbb.mp4",
            },
          ];
          const stmt = db.prepare(
            "INSERT INTO videos (title, thumbnail, price, video_url) VALUES (?, ?, ?, ?)",
          );
...
app.use(
  session({
    secret: process.env.SESSION_SECRET || "your_default_secret_change_this", // Set this in Replit Secrets
    resave: false,
    saveUninitialized: false,
    cookie: { secure: false }, // Set to true if using HTTPS (Replit provides HTTPS)
  }),
);
...
app.get("/videos/:id", (req, res) => {
  ...
  db.get("SELECT * FROM videos WHERE id = ?", [videoId], (err, video) => {
    ...
    if (req.session.user) {
      db.get(
        "SELECT 1 FROM purchases WHERE user_id = ? AND video_id = ?",
...
app.post("/videos/:id/purchase", isAuthenticated, (req, res) => {
  ...
  db.run(
    "INSERT OR IGNORE INTO purchases (user_id, video_id) VALUES (?, ?)",
    [userId, videoId],
    function (err) {
      if (err) {
      ...
      console.log(
        `Simulated successful purchase recorded for user ${userId} and video ${videoId}`,
      );
